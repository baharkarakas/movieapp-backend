...
import movieapp.dto.WatchlistDtos.AddWatchlistRequest;
...
public class WatchlistController {
    ...
    public List<WatchlistItemResponse> all(Authentication auth) {
        ...
        return watchlistRepo.findAllByUserEmailWithDetails(email).stream()
                .map(this::toResponse)
                .toList();
    }
    ...
    public ResponseEntity<?> byCategory(@PathVariable Long categoryId, Authentication auth) {
        ...
        return ResponseEntity.ok(
                watchlistRepo.findAllByUserEmailAndCategoryIdWithDetails(email, categoryId).stream()
                        .map(this::toResponse)
                        .toList()
        );
    }
